...
/**
 * Header — fixed top, transparent over hero, blurred bg on scroll.
 * Per design system §5 navigation spec.
 *
 * Mobile: hamburger opens full-screen overlay in --color-bg-elevated.
 */
          ...
          <nav className="hidden md:flex items-center gap-8">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm font-medium tracking-tight text-text-secondary hover:text-text-primary transition-colors"
              >
                {link.label}
              </Link>
            ))}
            ...
            className="hidden md:inline-flex items-center px-6 py-2.5 rounded-full bg-text-primary text-text-inverse text-sm font-medium tracking-tight hover:scale-[1.02] transition-transform"
          >
        ...
        <div className="fixed inset-0 z-40 bg-bg-elevated md:hidden pt-16 overflow-y-auto">
          <nav className="px-6 py-12 flex flex-col gap-6">
            ...
            <div className="border-t border-border-subtle pt-6 flex flex-col gap-4">
              ...
              rel="noopener noreferrer"
              onClick={() => setMobileOpen(false)}
              className="mt-4 inline-flex items-center justify-center px-8 py-4 rounded-full bg-text-primary text-text-inverse text-base font-medium tracking-tight w-full"
            >
              Book a Strategy Call
            </a>
          </nav>
